            ...
            <table id="tabelBorrowing">
                ...
                @php
                    ...
                    $otpExpired = $borrow->otp_expired_at
                        ? \Carbon\Carbon::parse($borrow->otp_expired_at)->isPast()
                        : false;
                    ...
                    if ($status === 'Menunggu OTP' && $borrow->otp_expired_at) {
                        $otpSisaDetik = max(0, (int) now()->diffInSeconds(
                            \Carbon\Carbon::parse($borrow->otp_expired_at), false
                        ));
                    }
                ...
                <tr data-nama="{{ strtolower($borrow->anggota->nama_lengkap ?? '') }}"
                    ...
                    <td>
                        @if($status === 'Menunggu OTP')
                            <span style="font-family:monospace;font-weight:700;color:#1fcf8e;font-size:14px;letter-spacing:1px;">
                                {{ $borrow->kode_otp }}
                            </span>
                        @else
                    ...
                    <td class="text-muted">
                        {{ $borrow->waktu_booking ? \Carbon\Carbon::parse($borrow->waktu_booking)->format('d M, H:i') : '-' }}
                    </td>
                    <td class="text-muted">
                    ...
                    <td>
                        @if($borrow->batas_pengembalian)
                            <span style="color:{{ $terlambat ? '#e53e3e' : '#2d3748' }};font-weight:{{ $terlambat ? '700' : '400' }};">
                                ...
                                @if($terlambat)
                                    <br><small style="color:#e53e3e;font-size:11px;">{{ $hariTerlambat }} hari terlambat</small>
                                @endif
                            </span>
                        @else
                    ...
                    <td>
                        ...
                        @else
                            <span style="color:#a0aec0;">Rp 0</span>
                        @endif
                    </td>
                    <td>
                        @if($status === 'Menunggu OTP')
                            <span class="status pending" style="background:#fff3e0;color:#e65100;white-space:nowrap;">Menunggu OTP</span>
                        @elseif($status === 'Dipinjam' && $terlambat)
                            <span class="status late" style="white-space:nowrap;">Terlambat</span>
                        @elseif($status === 'Dipinjam')
                            <span class="status pending">Dipinjam</span>
                        @elseif($status === 'Selesai')
                            <span class="status done">Selesai</span>
                        @elseif($status === 'Kadaluarsa')
                            <span class="status" style="background:#f7fafc;color:#718096;padding:5px 12px;border-radius:20px;font-size:12px;">Kadaluarsa</span>
                        @else
                            <span class="status late">{{ $status }}</span>
                        @endif
                    </td>
...
<div id="modal-peminjaman"
     onclick="tutupModalPeminjaman(event)"
    ...
    <div style="background:white;border-radius:16px;width:100%;max-width:600px;padding:24px;box-shadow:0 12px 30px rgba(0,0,0,0.15);margin:auto;">
        ...
        <div style="background:#f8fafc;border-radius:12px;padding:16px;margin-bottom:20px;display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div>
                ...
                <p id="modal-anggota" style="font-size:14px;font-weight:700;color:#1a202c;"></p>
                <span id="modal-anggota-id" style="background:#edf2f7;color:#718096;font-size:11px;padding:2px 8px;border-radius:4px;"></span>
            </div>
            <div>
                <p style="font-size:11px;color:#a0aec0;font-weight:600;letter-spacing:0.05em;margin-bottom:4px;">PETUGAS</p>
                <p id="modal-petugas" style="font-size:14px;font-weight:700;color:#1a202c;"></p>
            </div>
            <div>
                <p style="font-size:11px;color:#a0aec0;font-weight:600;letter-spacing:0.05em;margin-bottom:4px;">TGL PINJAM</p>
                <p id="modal-tgl-pinjam" style="font-size:14px;color:#2d3748;"></p>
            </div>
            <div>
                <p style="font-size:11px;color:#a0aec0;font-weight:600;letter-spacing:0.05em;margin-bottom:4px;">BATAS KEMBALI</p>
                <p id="modal-batas" style="font-size:14px;font-weight:700;"></p>
            </div>
        </div>
        ...
        <div id="denda-section" style="display:none;background:#fff5f5;border:1px solid #fed7d7;border-radius:12px;padding:16px;margin-bottom:20px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                <div>
                    <p style="font-size:11px;color:#a0aec0;margin-bottom:3px;">HARI TERLAMBAT</p>
                    <p id="modal-hari-terlambat" style="font-size:18px;font-weight:800;color:#e53e3e;"></p>
                </div>
                <div>
                    <p style="font-size:11px;color:#a0aec0;margin-bottom:3px;">TOTAL DENDA</p>
                    <p id="modal-total-denda" style="font-size:18px;font-weight:800;color:#e53e3e;"></p>
                </div>
            </div>
            <p style="font-size:11px;color:#718096;margin-top:8px;">
                Perhitungan: Rp 1.000 × <span id="modal-hari-x"></span> hari × <span id="modal-buku-x"></span> buku
            </p>
        </div>

        {{-- ===== SECTION: MENUNGGU OTP ===== --}}
        <div id="otp-section" style="display:none;">
            <div style="background:#f0fff4;border:2px dashed #1fcf8e;border-radius:12px;padding:20px;text-align:center;margin-bottom:20px;">
                ...
                <input type="text" id="input-otp-langsung" maxlength="6"
                       placeholder="Ketik 6 karakter OTP"
                ...
                <p style="font-size:12px;color:#718096;display:flex;align-items:center;justify-content:center;gap:5px;margin-top:12px;">
                    ...
                    OTP kadaluarsa dalam <strong style="color:#e53e3e;" id="otp-timer">--:--</strong>
                </p>
            </div>
            <div style="background:#f8fafc;border-radius:12px;padding:16px;margin-bottom:20px;">
                ...
                <div style="display:flex;gap:20px;flex-wrap:wrap;">
                    <label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;">
                        <input type="radio" name="kondisi_fisik" value="Sangat Baik" checked style="accent-color:#1fcf8e;"> Sangat Baik
                    </label>
                    <label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;">
                        <input type="radio" name="kondisi_fisik" value="Rusak Ringan" style="accent-color:#1fcf8e;"> Rusak Ringan
                    </label>
                    <label style="display:flex;align-items:center;gap:8px;font-size:14px;cursor:pointer;">
                        <input type="radio" name="kondisi_fisik" value="Hilang" style="accent-color:#1fcf8e;"> Hilang
                    </label>
                </div>
            </div>
        ...
        <div id="btn-tutup-saja" style="display:none;text-align:right;">
            <button onclick="tutupModal()"
                    style="padding:11px 30px;background:#f7fafc;border:1px solid #e2e8f0;border-radius:10px;cursor:pointer;font-size:14px;color:#4a5568;">
                Tutup
            </button>
        </div>
...
function filterTabel() {
    ...
    document.querySelectorAll('#tabelBorrowing tbody tr').forEach(row => {
        ...
        const cocok     = nama.includes(keyword) && (status === '' || rowStatus === status);
