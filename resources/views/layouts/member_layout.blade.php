...
<head>
    ...
    <!-- Mengacu ke public/dashboard_assets/style.css -->
    <link rel="stylesheet" href="{{ asset('dashboard_assets/style.css') }}">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css">
    <style>
        /* CSS Tambahan untuk memastikan Sidebar Terkunci (Locked) */
...
<div class="dashboard-container">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-menu" style="margin-top: 40px;">
            ...
            <a href="{{ route('member.keranjang') }}" class="menu-item {{ request()->routeIs('member.keranjang') ? 'active' : '' }}" style="display: block; padding: 12px; color: white; text-decoration: none;">
                <i class="fa-solid fa-cart-shopping"></i> <span>Keranjang</span>
            </a>
            <a href="{{ route('member.peminjaman') }}" class="menu-item {{ request()->routeIs('member.peminjaman') ? 'active' : '' }}" style="display: block; padding: 12px; color: white; text-decoration: none; border-radius: 8px; margin-bottom: 5px;">
                <i class="fa-solid fa-rectangle-list"></i> <span>Peminjaman Saya</span>
            </a>
            <a href="{{ route('member.riwayat') }}" class="menu-item {{ request()->routeIs('member.riwayat') ? 'active' : '' }}" style="display: block; padding: 12px; color: white; text-decoration: none; border-radius: 8px; margin-bottom: 5px;">
                <i class="fa-solid fa-clock-rotate-left"></i> <span>Riwayat</span>
            </a>
            <a href="{{ route('member.profil') }}" class="menu-item {{ request()->routeIs('member.profil') ? 'active' : '' }}" style="display: block; padding: 12px; color: white; text-decoration: none; border-radius: 8px; margin-bottom: 5px;">
        ...
        <div class="sidebar-footer" style="margin-top: auto; padding-bottom: 20px;">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" style="width: 100%; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; padding: 10px; border-radius: 10px; cursor: pointer;">
                    Logout
                </button>
            </form>
        </div>
