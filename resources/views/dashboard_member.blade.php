...
<div style="display:grid;grid-template-columns:1fr 320px;gap:25px;">
        ...
        <div style="background:white;border-radius:15px;padding:20px;border:1px solid #edf2f7;">
            ...
            @php
                ...
                $hariSisa   = $trx->batas_pengembalian
                    ? (int) now()->diffInDays(\Carbon\Carbon::parse($trx->batas_pengembalian), false) : null;
            @endphp
            <div style="display:flex;align-items:center;gap:15px;padding:15px 0;border-bottom:1px solid #f7fafc;">
                @if($firstBuku?->cover)
                    <div style="width:55px;height:75px;border-radius:6px;overflow:hidden;flex-shrink:0;">
                        <img src="{{ asset('storage/' . $firstBuku->cover) }}" style="width:100%;height:100%;object-fit:cover;">
                    </div>
                @else
                ...
                <div style="flex:1;">
                    ...
                    <small style="color:#a0aec0;">
                        {{ $trx->status_transaksi === 'Menunggu OTP'
                            ? 'OTP: '.$trx->kode_otp
                            : 'Dipinjam: '.$tglPinjam.' - '.$tglKembali }}
                    </small>
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        ...
        <div style="background:white;border-radius:15px;padding:20px;border:1px solid #edf2f7;border-bottom:3px solid #4299e1;">
            ...
            <small style="color:#a0aec0;">Transaksi</small>
        </div>
        ...
        {{-- SHORTCUT KE PROFIL --}}
        <div style="background:white;border-radius:15px;padding:20px;border:1px solid #edf2f7;">
            <p style="margin:0 0 5px;font-size:12px;color:#718096;">Status Akun</p>
            <p style="margin:0 0 15px;font-size:14px;font-weight:600;color:#1fcf8e;">
                {{ $anggota?->status_verifikasi ?? 'Incomplete' }}
            </p>
            <p style="margin:0 0 5px;font-size:12px;color:#718096;">Terdaftar Sejak</p>
            <p style="margin:0 0 15px;font-size:14px;font-weight:600;color:#2d3748;">
